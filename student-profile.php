...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_semester'])) {
    ...
    if (empty($semester)) {
        ...
        try {
            $update_stmt = $pdo->prepare("UPDATE students SET semester = ? WHERE id = ?");
            if ($update_stmt->execute([$semester, $student['id']])) {
        ...
        if (!in_array($file_type, $allowed_types)) {
            ...
            if (empty($error_message)) {
                ...
                if (move_uploaded_file($_FILES['profile_picture']['tmp_name'], $destination)) {
                    try {
                        $update_stmt = $pdo->prepare("UPDATE students SET profile_picture = ? WHERE id = ?");
                        if ($update_stmt->execute([$destination, $student['id']])) {
